...
　　Google Chart API官方数学公式文档地址： https://developers.google.com/chart/infographics/docs/formulas
...
```
    ...
    x_test = scaler.transform(np.array([1650,3]))	# 有错误，不知道错在哪
```
- 线性模型拟合
```
    # 线性模型拟合
...
因此，目标函数等价于：  
![\min_{w,b}\max_{\lambda_{i}>0}L(w,b,\lambda)](http://chart.apis.google.com/chart?cht=tx&chl={\min_{w,b}\max_{\lambda_{i}>0}L(w,b,\lambda)})  
上式的对偶问题正好相反：  
![\max_{\lambda_{i}>0}\min_{w,b}L(w,b,\lambda)](http://chart.apis.google.com/chart?cht=tx&chl={\max_{\lambda_{i}>0}\min_{w,b}L(w,b,\lambda)})  
...
然后通过![b=\frac{1}{2}(\max_{y=-1}(w^{T}x_{i}+\min_{y=1}(w^{T}x_{i})))](http://chart.apis.google.com/chart?cht=tx&chl={b=\frac{1}{2}(\max_{y=-1}(w^{T}x_{i}%2B\min_{y=1}(w^{T}x_{i})))))（为了保证约束条件成立）  
即可求出b。从而，得到超平面函数如下：  
...
将数据映射到新的空间，看似解决了问题，实际并不是。由于低维映射到高维，有可能产生维度灾难。在高维上计算内积将是非常困难的甚至无法计算。因此引入了核（kernel）函数，使计算发生在低维。  
- 核函数为什么能起到这样的作用？  
从最简单的二维平面样本点的例子开始，如下图![enter description here][68]  
对应的超平面应该是一个圆圈，可以用一个二次曲线表示，方程如下![enter description here][69]  
可以映射到五维空间，新坐标系可写为![enter description here][70]  
映射后三维空间显示![enter description here][66]  
设两个向量![x_{1}=(\eta_{1},\eta_{2})^{T}](http:://chart.apis.google.com/chart?cht=tx&chs=1x0&chf=bg,s,FFFFFF00&chco=000000&chl=J(%5Ctheta%20)%20%3D%20C%5Csum%5Climits_%7Bi%20%3D%201%7D%5Em%20%7B%5B%7By%5E%7B(i)%7D%7D%5Ccos%20%7Bt_1%7D(%7B%5Ctheta%20%5ET%7D%7Bf%5E%7B(i)%7D%7D)%20%2B%20(1%20-%20%7By%5E%7B(i)%7D%7D)%5Ccos%20%7Bt_0%7D(%7B%5Ctheta%20%5ET%7D%7Bf%5E%7B(i)%7D%7D)%7D%20%5D%20%2B%20%5Cfrac%7B1%7D%7B2%7D%5Csum%5Climits_%7Bj%20%3D%201%7D%5E%7B%5Ctext%7Bn%7D%7D%20%7B%5Ctheta%20_j%5E2%7D)
和![x_{2}=(\xi_{1},\xi_{2})^{T}](http:://chart.apis.google.com/chart?cht=tx&chl={x_%7B2%7D=(%5Cxi_%7B1%7D,%5Cxi_%7B2%7D)%5E%7BT%7D})
映射后的内积为：  
