...
* [机器学习算法Python实现](#机器学习算法python实现)
	...
	* [四、SVM支持向量机](#四svm支持向量机)
		...
		* [3、补充1（进一步理解）](#3补充1)
		* [4、补充2（求解）](#4补充2)
		* [5、SVM Kernel（核函数）](#5svm-kernel核函数)
		* [6、使用中的模型代码](#6使用scikit-learn中的svm模型代码)
		* [7、运行结果](#7运行结果)
...
设两个向量![x_{1}=(\eta_{1},\eta_{2})^{T}](http://chart.apis.google.com/chart?cht=tx&chl=x_%7B1%7D%3D(%5Ceta_%7B1%7D%2C%5Ceta_%7B2%7D)%5E%7BT%7D)
...
这个核可以将原始空间映射为无穷维空间。不过，如果σ选得很大的话，高次特征上的权重实际上衰减得非常快，
所以实际上（数值上近似一下）相当于一个低维的子空间；反过来，如果σ选得很小，则可以将任意的数据映射为线性可分。
