...
![l(\theta)=\log{L(\theta)}\\\quad =\sum_{i=1}^{m}\left(y^{(i)}\log{h_{\theta}(x^{(i)})}+(1-y^{(i)})\log{(1-h_{\theta}(x^{(i)}))} \right)](http://chart.apis.google.com/chart?cht=tx&chl={l(\theta)=\log{L(\theta)}\\=\sum_{i=1}^{m}\left(y^{(i)}\log{h_{\theta}(x^{(i)})}%2B(1-y^{(i)})\log{(1-h_{\theta}(x^{(i)}))}\right)})
